...
def data_gen():
    """ 
    Generate fake training and testing images
    use this to test your ML model code flow
    """
    grain = 4.0
    L = np.arange(0,129*grain, grain)
    gridX, gridY = np.meshgrid(L,L)
    px, py = (80, 100)
    np.cos(2*np.pi*X/px) * np.sin(2*np.
